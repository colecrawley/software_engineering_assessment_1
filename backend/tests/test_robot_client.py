...
@pytest.mark.asyncio
async def test_move_robot_negative_x():
    client = RobotClient()
...
@pytest.mark.asyncio
async def test_move_robot_float_x():
    client = RobotClient()
...
@pytest.mark.asyncio
async def test_move_robot_boundary_zero():
    client = RobotClient()
...
@pytest.mark.asyncio
async def test_get_status_sets_error_on_failure():
    client = RobotClient()
...
def test_factory_test_client_uses_transport():
...
def test_get_command_history_calls_query():
    logger = AuditLogger()
    mock_db = MagicMock()
